var kripton = function () {
  ...
  return {
    init: function () {
      var o;
      t(), jQuery("#menu").length > 0 && $("#menu").metisMenu(), jQuery(".metismenu > .mm-active ").each(function () {
        ...
        }(), n(), jQuery("#ckeditor").length > 0 && ClassicEditor.create(document.querySelector("#ckeditor"), {}).then(e => {
          window.editor = e
        }).catch(e => {
    ...
    load: function () {
      t(), jQuery(".default-select").length > 0 && jQuery(".default-select").selectpicker(),
        function () {
          const e = $(".image-select");
          e.find("option").each((e, t) => {
            const n = $(t),
              o = n.attr("data-thumbnail");
            o && n.attr("data-content", "<img src='%i'/> %s".replace(/%i/, o).replace(/%s/, n.text())), console.warn("option:", e, n)
          }), e.selectpicker()
        }(), jQuery("body").append('<script id="DZScript" src="js/w4-global.js?btn_dir=right"><\/script>')
    },
    resize: function () {
      n()
    }
...
jQuery(document).ready(function () {
  $('[data-bs-toggle="popover"]').popover(), kripton.init()
